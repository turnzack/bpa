...
const inputDir = path.join(__dirname, '..', 'data', 'prix');
const outputFile = path.join(__dirname, '..', 'data', 'bibliotheque_prix.json');
...
function parseLuaContent(content: string) {
...
function processNode(node: any) {
    ...
    if (node.type === 'ARTICLE') {
        ...
        if (explicitUnit) {
            if (explicitUnit === 'm') unite = 'm';
            else if (explicitUnit === 'U') unite = 'U';
            else unite = explicitUnit;
        } else {
            const lowerNom = cleanNom.toLowerCase();
            if (lowerNom.match(/\b(m2|m²)\b/)) unite = 'm2';
            else if (lowerNom.match(/\b(m3|m³)\b/)) unite = 'm3';
            else if (lowerNom.match(/\bml\b/)) unite = 'ml';
            else if (lowerNom.match(/\bforfait\b/)) unite = 'Forfait';
            else if (lowerNom.match(/\bens\b/)) unite = 'Ens';
            else if (lowerNom.match(/\bh\b/)) unite = 'H';
        }
...
const finalLibrary: Record<string, any> = {};

try {
    const files = fs.readdirSync(inputDir).filter(f => f.endsWith('.lua'));
    ...
    fs.writeFileSync(outputFile, JSON.stringify(finalLibrary, null, 2), 'utf8');
